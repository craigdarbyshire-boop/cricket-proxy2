...
MATCHLIST_URL = (
    ...
    '&page=0&showFixtures=true&showLive=true&showResults=false'
...
def fetch_nvplay_scorecard(match_id):
    url = f'https://w-api.ecb.nvplay.net/api/scorecard/{match_id}?idType=nvplay&customerId={ECB_CUSTOMER_ID}&stats=true&commentary=false'
...
def get_cached_or_fetch(match_id):
    ...
    try:
        ...
        data = {'error': str(e), 'innings': [], 'match_title': '', 'status': 'Error',
...
def matches():
    try:
        ...
        resp.raise_for_status()
        data = resp.json()
        # Normalise to a simple list
        match_list = []
        items = data if isinstance(data, list) else data.get('Matches', data.get('matches', data.get('Items', [])))
        for m in items:
            match_list.append({
                'id': m.get('MatchId') or m.get('Id') or m.get('matchId', ''),
                'title': m.get('MatchTitle') or m.get('Title') or m.get('matchTitle', ''),
                'team1': m.get('Team1Name') or m.get('HomeTeamName', ''),
                'team2': m.get('Team2Name') or m.get('AwayTeamName', ''),
                'competition': m.get('CompetitionName') or m.get('Competition', ''),
                'venue': m.get('VenueName', ''),
                'status': m.get('MatchStatus') or m.get('Status', ''),
                'is_live': m.get('IsLive', False),
                'score1': m.get('Team1Scores') or m.get('HomeScore', ''),
                'score2': m.get('Team2Scores') or m.get('AwayScore', ''),
                'start': m.get('StartDateFormatted') or m.get('StartDateTime', ''),
            })
...
@app.route('/')
def index():
    return '<html><body style="font-family:sans-serif;padding:30px;background:#0a1a0f;color:#f0ead8"><h2>&#x1F3CF; Cricket Proxy</h2><p style="color:#7de0a0">Running OK - NV Play API mode</p></body></html>'
